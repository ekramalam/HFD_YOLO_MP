...
def draw_text_panel(frame, lines, position="top_left"):
    """
    Draw fixed-position semi-transparent debug panel.
    """
...
class FallEngine:
    ...
    def __init__(self, model_path='yolov12n1.pt', confidence=0.5, fall_threshold=0.1):
        ...
        self.bbox_padding_ratio = 0.15   # 15% margin for filtering
        ...
        self.mediapipe_padding_ratio = 0.00   # 5% padding for MediaPipe crop
        ...
        # self.last_fall_criteria = []   # Stores criteria met for last fall
        self.last_fall_criteria = []   # Stores criteria met for last fall
        self.all_fall_criteria = []    # Stores ALL criteria with True/False status
        ...
        # self.fall_threshold = 0.2  # Threshold for fall detection
        
        self.angle_threshold = 45  # Threshold for body angle
        self.angle_change_threshold = 15  # Threshold for angle change between frames
        # self.aspect_ratio = 1.5
        self.confirm_frames = 2
        # self.top_threshold = 0.5 #URFD
        self.top_threshold = 0.4 #GMDCSA-24

        # self.head_ground_threshold = 0.73 #URFD
        self.head_ground_threshold = 0.7 #GMDCSA-24
        self.vertical_drop_frames = 8
        self.angle_change_frames = 5
        self.fps = 30   # default fps (will be overwritten by video fps)
    ...
    def analyze_pose(self, frame, person_box):
        ...
        # Extract padded crop for MediaPipe
    ...
    def calculate_pose_features(self, landmarks, person_box):
        ...
        nose_y_box = landmarks[0][1]          # normalized inside person box
        ...
        if len(self.prev_poses) >= self.vertical_drop_frames:
            ...
            if len(self.prev_poses) >= frames + 1:

                prev_pose = self.prev_poses[-frames]
                prev_pose2 = self.prev_poses[-frames-1]

                prev_velocity_y = (
                    prev_pose["mid_shoulder_y"] - prev_pose2["mid_shoulder_y"]
                ) / (frames * dt)

                acceleration = (velocity_y - prev_velocity_y) / dt
                
     

                if len(self.prev_poses) >= frames + 3:

                    pose_a = self.prev_poses[-frames]
                    pose_b = self.prev_poses[-frames-1]
                    pose_c = self.prev_poses[-frames-2]
                    pose_d = self.prev_poses[-frames-3]

                    v1 = (pose_a["mid_shoulder_y"] - pose_b["mid_shoulder_y"]) / (frames * dt)
                    v2 = (pose_b["mid_shoulder_y"] - pose_c["mid_shoulder_y"]) / (frames * dt)
                    v3 = (pose_c["mid_shoulder_y"] - pose_d["mid_shoulder_y"]) / (frames * dt)
                    #
    ...
    # def detect_fall(self, pose_features):
        """Detect if a person has fallen and record which criteria were met."""
        # print("detect_fall CALLED")

        if not pose_features:
            return False

        # Reset criteria log for this frame
        ...
        acc = abs(pose_features.get("acceleration", 0))
        angle_change = 0
        ...
        if len(self.prev_poses) >= self.angle_change_frames:
            ...
            if mid_hipX_mid_shldrX > 0:   # right bend
        ...
        # ---------- Criterion 6: Knee-Ankle collapse ----------
        ...
        # ---------- Criterion 8: High vertical velocity ----------
    ...
    def process_frame(self, frame):
        ...
        for i, box in enumerate(person_boxes):
            ...
            # x1_bbox_left, y1_bbox_top, x2_bbox_right, y2_bbox_bottom = box
        ...
        for person_id, landmarks, pose_features, (x1_bbox_left, y1_bbox_top, x2_bbox_right, y2_bbox_bottom) in pose_data:
            ...
            # is_fall = self.detect_fall(pose_features)
            is_fall = self.detect_fall(pose_features, person_id)
            # --------- DEBUG OVERLAY ----------
            # aspect_ratio = pose_features.get("aspect_ratio", 0)
            velocity_y = pose_features.get("velocity_y", 0)
            acc = pose_features.get("acceleration", 0)

            vertical_drop_norm = pose_features.get("vertical_drop_norm", 0)

           
            if is_fall:
                # fall_type = self.classify_fall_type(pose_features) or "slip_and_fall"
        ...
        # ---------- DEBUG PANEL (CRITERIA FORMAT) ----------
        COLOR_PASS = (0,255,0)       # green → criterion satisfied
        COLOR_OVERRIDE = (0,255,255) # yellow → override active
        COLOR_FAIL = (250,250,250)   # white → not satisfied
        COLOR_INFO = (200,200,200)   # gray → dependent info
        ...
        # debug_lines.append(f"Fallen Person ID: {fall_data['fallen_ids']}")
        ...
        for person_id, _, pose_features, _ in pose_data:
            ...
            if head_override:
                color = (0,255,255)     # yellow override
            else:
                color = (250,250,250)
            ...
            # baseline = self.baseline_height if self.baseline_height else 1
            body_height_ank_to_head = pose_features.get("body_height_ank_to_head", 1)
            # shoulder_y = pose_features.get("mid_shoulder_y", 0)
            ...
            if len(self.prev_poses) >= self.angle_change_frames:
                prev = self.prev_poses[-self.angle_change_frames]
                # angle_change = pose_features["angle_signed"] - prev["angle_signed"]
    ...
    def track_persons(self, frame, person_boxes):
        ...
        Args:
            frame: Input frame
            person_boxes: List of person bounding boxes [(x1_bbox_left, y1_bbox_top, x2_bbox_right, y2_bbox_bottom), ...]
